...
class LearnableFourierEmbedding(torch.nn.Module):
    def __init__(self, num_frequencies=16, embed_dim=32):
        super().__init__()
        self.frequencies = torch.nn.Parameter(torch.randn(num_frequencies))  # Learnable frequencies
        self.linear = torch.nn.Linear(num_frequencies * 2, embed_dim)

    def forward(self, t):
        t = t.unsqueeze(-1)  # (B, 1)
        embeddings = torch.cat([torch.sin(self.frequencies * t), torch.cos(self.frequencies * t)], dim=-1)
        return self.linear(embeddings)  # (B, embed_dim)
...
class SelfAttentionBlock(nn.Module):
    def __init__(self, dim, dim_head=32, num_heads=4, dropout=0.0, max_seq_len=128):
        super().__init__()
        # treat as an image
    ...
    def forward(self, x, mask=None):
        ...
        sim = (
            torch.einsum("b h i d, b h j d -> b h i j", q, k) * q.shape[-1] ** -0.5
        )  # B x head x horizon x horizon
        if mask is not None:
            mask_mat = repeat(
                mask, "b j -> b h i j", h=self.num_heads, i=sim.shape[-2]
            )  # convert to matrix with last column to be masked by 0
...
class ResDoubleConv2d(nn.Module):
    def __init__(self, input_chn, hidden_chn, num_groups, dropout):
        ...
        self.residual_conv = (
            nn.Conv2d(input_chn, hidden_chn, kernel_size=1)
            if input_chn != hidden_chn
            else nn.Identity()
        )
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        SquareRearrange2D(
                            Residual(PreNorm(norm_fn(out_chn), attn_block(out_chn)))
                        ),
...
class ViT1D(nn.Module):
    def __init__(
        ...
        self.to_embed_dim = getattr(module, cfg.obs_to_embed_model.name)(
            **cfg.obs_to_embed_model.args
        )  # ObstacleToEmbedViT or ObstacleToEmbedMLP

        self.transformer = Transformers(
            num_layers, embed_dim, dim_head, num_heads, dropout, max_seq_len
        )
...
class ObstacleToEmbedViT(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(
            x, "b h (l d) -> (b h) l d", d=self.input_dim + 1
        )  # fake batch size
...
class Transformers(nn.Module):
    def __init__(
        self, layer_nums, embed_dim, dim_head, num_heads, dropout, max_seq_len
    ):
        super().__init__()
        self.layers = nn.ModuleList([])
        attn_block = partial(
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        Rearrange(
                            "b h c -> b c h"
                        ),  # the attention block receives input in a different format
