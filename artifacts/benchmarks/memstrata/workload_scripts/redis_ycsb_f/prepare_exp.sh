...
sed -i 's/operationcount=.*/operationcount=15000000/' workloads/workloadf
sed -i 's/requestdistribution=.*/requestdistribution=zipfian/' workloads/workloadf

python2 ./bin/ycsb load redis -s -P workloads/workloadf -p "redis.host=127.0.0.1" -p "redis.port=6379" -p "threadcount=3"
