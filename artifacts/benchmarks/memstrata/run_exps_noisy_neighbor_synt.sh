...
for idx in "${!vm_arr[@]}"; do
  	vm_idx="${vm_arr[$idx]}"
	rss_MB=${rss_MB_arr[$idx]}
	vm_size_MB=${vm_size_MB_arr[$idx]}
	num_vcpus=${num_vcpus_arr[$idx]}
	vm_core=${vm_core_arr[$idx]}
...
# Signal the orchestrator to start
...
for idx in "${!vm_arr[@]}"; do
	...
	ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR ubuntu@$vm_ip \
    	"cd /home/ubuntu/; source ~/.bashrc; GAPBS_DIR=/home/ubuntu/shared/gapbs GAPBS_GRAPH_DIR=/home/ubuntu/shared/gapbs/benchmark/graphs bash ./run_exp.sh > /home/ubuntu/result_run_exp_stdout.txt 2> /home/ubuntu/result_run_exp_stderr.txt < /dev/null &"
done
...
last_idx=$(( ${#vm_arr[@]} - 1 ))
last_vm_idx="${vm_arr[$last_idx]}"
last_vm_ip="${vm_ip_arr[$last_idx]}"

# Wait for the last VM to finish
while [ ! -z "$(ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR ubuntu@$last_vm_ip "pgrep -f run_exp")" ]; do
	sleep 30
done

# Kill the processes on the remaining VMs
for idx in "${!vm_arr[@]}"; do
...
for idx in "${!vm_arr[@]}"; do
	...
	scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR \
		ubuntu@$vm_ip:/home/ubuntu/result_app_perf.txt result_app_perf_${vm_idx}_${exp_name}.txt || true
done
