...
static inline void ALWAYS_INLINE OPTIMIZE3 stress_nt_store128(__uint128_t *addr, __uint128_t value)
{
 ...
 */
    ...
    defined(HAVE_BUILTIN_NONTEMPORAL_STORE)
/* Clang non-temporal stores */
static inline void ALWAYS_INLINE OPTIMIZE3 stress_nt_store64(uint64_t *addr, uint64_t value)
{
	__builtin_nontemporal_store(value, addr);
}
 ...
 */
    ...
    defined(HAVE_BUILTIN_NONTEMPORAL_STORE)
/* Clang non-temporal stores */
static inline void ALWAYS_INLINE OPTIMIZE3 stress_nt_store32(uint32_t *addr, uint32_t value)
{
	__builtin_nontemporal_store(value, addr);
}
...
static inline void ALWAYS_INLINE OPTIMIZE3 stress_nt_store_double(double *addr, double value)
{
	if (sizeof(double) == sizeof(long long int)) {
		void *vptr = (void *)&value;	/* avoid type punning warnings */
