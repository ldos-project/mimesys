...
Tensor3D ReadHDF5File(std::filesystem::path file_path) {
    const std::string dataset_name = "execution_plan";
    ...
    size_t dim0 = dims_out[0];
    size_t dim1 = dims_out[1];
    size_t dim2 = dims_out[2];

    std::vector<float> buffer(dim0 * dim1 * dim2);
    dataset.read(buffer.data(), H5::PredType::NATIVE_FLOAT);

    Tensor3D tensor(dim0, std::vector<std::vector<double>>(dim1, std::vector<double>(dim2)));

    size_t idx = 0;
    for (size_t i = 0; i < dim0; ++i) {
      for (size_t j = 0; j < dim1; ++j) {
        for (size_t k = 0; k < dim2; ++k) {
            tensor[i][j][k] = buffer[idx++];
        }
      }
    }

    std::cerr << "Tensor loaded successfully. First element: " << tensor[0][0][0] << std::endl;

    return tensor;
}


MimesysExecutionPlan ReadExecutionPlanFile(std::filesystem::path file) {
  Tensor3D execution_plan = ReadHDF5File(file);
  MimesysExecutionPlan plan;
  ...
  //     double value;
  //     CHECK(absl::SimpleAtod(col, &value)) << "Invalid column: " << col << "\n";
  //     entry.push_back(value);
  //   }
  //   plan.execution_plan.push_back(std::move(entry));
  // }
  // return plan;
}

void RunBackgroundCommand(std::filesystem::path file, std::filesystem::path lock_file) {
  std::string program_path = GetFleetbenchRuntimePath("mimesys/memstrata_commands") + "/background";
  std::string command = "bash " + program_path + " " + file.string() + " " + lock_file.string();
  std::thread([command]() {
...
void RunMemstrataAndWait(std::filesystem::path file, std::filesystem::path lock_file) {
  std::ofstream lockfile_out(lock_file);
  lockfile_out << "1";
  lockfile_out.close();

  // RunBackgroundCommand(file, lock_file);

  while (true) {
    std::ifstream lockfile_in(lock_file);
    std::string status;
    if (std::getline(lockfile_in, status) && status == "1") {
      break;
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    std::cerr << "Waiting for memstrata command to complete..." << std::endl;
  }
}
